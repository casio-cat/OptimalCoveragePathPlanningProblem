...
void ReadProblem()
{
    ...
    while ((Line = ReadLine(ProblemFile))) {
        if (!(Keyword = strtok(Line, Delimiter)))
            continue;
...
{
    if (!(Name = Copy(strtok(0, Delimiter))))
        eprintf("NAME: string expected");
}

static void Read_DIMENSION()
{
    char *Token = strtok(0, Delimiter);
...
static void Read_DISPLAY_DATA_TYPE()
{
    ...
    if (!(DisplayDataType = Copy(strtok(0, Delimiter))))
...
static void Read_EDGE_DATA_FORMAT()
{
    ...
    if (!(EdgeDataFormat = Copy(strtok(0, Delimiter))))
...
static void Read_EDGE_WEIGHT_FORMAT()
{
    ...
    if (!(EdgeWeightFormat = Copy(strtok(0, Delimiter))))
...
static void Read_EDGE_WEIGHT_TYPE()
{
    ...
    if (!(EdgeWeightType = Copy(strtok(0, Delimiter))))
...
static void Read_NODE_COORD_TYPE()
{
    ...
    if (!(NodeCoordType = Copy(strtok(0, Delimiter))))
...
static void Read_TYPE()
{
    ...
    if (!(Type = Copy(strtok(0, Delimiter))))
